...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handler(WrongParamException wrongParamException) {
        // 어떤 에러가 발생했는지.
        String message = "wrong parameter error";
        // 예외의 message를 통해 자세한 내용을 가져옴.
        String details = wrongParamException.getMessage();
        // ErrorResponse 객체를 만듦
        var errorResponse = new ErrorResponse(message, details);
        // ErrorResponse객체를 JSON으로 바꾼 것을 반환하면서 BAD_REQUEST(400)를 반환.
        return new ResponseEntity<>(errorResponse, HttpStatus.BAD_REQUEST);
